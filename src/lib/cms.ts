...
export async function getPublishedGalleryImages() {
  try {
    ...
    `;
    const items = data as unknown as GalleryImage[];
    setCached(cacheKey, items, LIST_CACHE_TTL_MS);
    return items;
  } catch (error) {
...
export async function getGalleryImageById(id: number) {
  await ensureGallerySchema();
  const data = await sql`SELECT * FROM gallery_images WHERE id = ${id} LIMIT 1`;
  return (data[0] as unknown as GalleryImage) ?? null;
}

export async function createGalleryImage(item: {
  image_url: string;
  alt?: string;
  caption?: string | null;
  display_order?: number;
  ...
  `;
  const nextOrder = Number((nextOrderRows[0] as { next: number }).next) || 1;
...
export async function updateGalleryImage(id: number, item: Partial<GalleryImage>) {
  await ensureGallerySchema();
  const current = await getGalleryImageById(id);
  if (!current) return null;

  const nextUrl = item.image_url ?? current.image_url;
  const data = await sql`
    UPDATE gallery_images SET
      image_url = ${nextUrl},
      alt = COALESCE(${item.alt ?? null}, alt),
      caption = COALESCE(${item.caption ?? null}, caption),
      display_order = COALESCE(${item.display_order ?? null}, display_order),
      is_published = COALESCE(${item.is_published ?? null}, is_published),
    ...
    RETURNING *
  `;
  if (item.image_url && item.image_url !== current.image_url && current.image_url) {
    await deleteFromR2(current.image_url);
  }
  clearCmsCache();
  return (data[0] as unknown as GalleryImage) ?? null;
}
...
export async function reorderGalleryImages(ids: number[]) {
  ...
  for (let i = 0; i < ids.length; i++) {
    await sql`
      ...
      SET display_order = ${i + 1}, updated_at = TIMEZONE('utc'::text, NOW())
      WHERE id = ${ids[i]}
    `;
  }
  clearCmsCache();
