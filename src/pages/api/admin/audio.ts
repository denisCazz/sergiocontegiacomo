...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  if (id) {
    const item = await getAudioPillola(Number(id));
    if (!item) return json({ error: 'Non trovato' }, 404);
    return json({ item });
  }

  const items = await getAudioPillole();
  return json({ items });
};
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const item = await createAudioPillola(body);
...
export const PATCH: APIRoute = async ({ request, url }) => {
  ...
  try {
    const body = await request.json();
    const item = await updateAudioPillola(id, body);
    return json({ item });
  } catch (err: any) {
...
export const DELETE: APIRoute = async ({ request, url }) => {
  ...
  const ok = await deleteAudioPillola(id);
